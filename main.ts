...
let pinInput = await inquirer.prompt(
    {name: "rPin", type: "number", message: "Please Enter Your Pin Number:" });
    ...
    let selectAns = await inquirer.prompt([{
        ...
        if (selectAns.select == 'Balance Inquiry') { console.log(`Your Balance Is : ${balance}`)}
        if (selectAns.select == 'Withdraw') {            
            let amount = await inquirer.prompt({ type:"number", message:"Please Enter Desire Amount", name:"rupee" })  
            if(amount.rupee <= balance){ console.log(chalk.bgGreen(`Take Your Cash From Dispenser Rupee: ${amount.rupee}`)), console.log(chalk.bgBlue(`Balance in Your Account Now Is: ${balance-amount.rupee}`)) }  
            if(amount.rupee >= balance){ console.log(chalk.red("\nAmount You Have Entered is insufficient What's in Your Account.")), console.log(chalk.redBright("Please Try Again Leter.")), console.log(chalk.green("HBL CLIFTON BR - 178565 \nGood Bye."))}
        }
        if(selectAns.select == "Fast Cash"){ const sAns = await inquirer.prompt ({  type:"list", name:"fselect",  message:"Select Your Desire Amount.",  choices:["5000","10000","15000","20000","25000","30000"] }) 
